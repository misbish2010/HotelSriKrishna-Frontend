  ...
  useEffect(() => {
    ...
    setFormData({
      guestInfo: {
        ...
        idType: "", // not provided in backend example
        idNumber: bookingDetails.customer_info?.identity || "",
...
//       );
//       if (res?.success) { toast.success("Booking cancelled"); onDone && onDone(); } else toast.error(res?.message || "Failed");
//     } catch (err) { console.error(err); toast.error("Server error"); }
//     setSubmitting(false);
//   };
//
//   const handleCheckInNow = async () => {
//     if (!window.confirm("Mark checked-in?")) return;
//     setSubmitting(true);
//     try {
//       const res = await checkInBooking(
//                     bookingDetails.booking_id,
//                   );
//       if (res?.success) { toast.success("Checked-In"); onDone && onDone(); } else toast.error(res?.message || "Failed");
//     } catch (err) { console.error(err); toast.error("Server error"); }
//     setSubmitting(false);
//   };
//
//   const handleCheckoutNow = async () => {
//     if (!window.confirm("Checkout now?")) return;
//     setSubmitting(true);
//     try {
//         console.log(formData.stayInfo.checkOut)
//       const res = await checkoutBooking(
//                     bookingDetails.booking_id,
//                     formData.stayInfo.duration,
//                     new Date(formData.stayInfo.checkOut).toISOString()
//                   );
//       if (res?.success) { toast.success("Checked out"); onDone && onDone(); } else toast.error(res?.message || "Failed");
//     } catch (err) { console.error(err); toast.error("Server error"); }
//     setSubmitting(false);
//   };
    ...
    switch (currentStep) {
      ...
      case 1:
          return <ManageStepStayInfo
              ...
              />
        //return <ManageStepStayInfo stayInfo={formData.stayInfo} onChange={(d) => updateFormData("stayInfo", d)} disableStayEditing={disableStayEditing} />;
      case 2:
  ...
  return (
    ...
    <CancelSettlementModal
      ...
      formData={formData}
      onConfirm={async (refundAmount, notes) => {
        ...
        if (refundAmount && refundAmount > 0) {
          updatedPayments.push({
            ...
            mode: "Refund",
            notes: notes || "Booking Cancelled - Refund",
            status: "refunded",
          });
        }
        ...
        // ✅ only after saving refund, cancel the booking
        //handleCancelBooking();
      }}
    />
    ...
    <CheckoutSettlementModal
      ...
      onConfirm={async (extraRefund, extraPayment, extraDiscount, paymentMode, notes, checkoutDate, duration) => {
        ...
        // ✅ only after saving payments, perform checkout
        //handleCheckoutNow();
      }}
    />
    ...
    <CheckInSettlementModal
      ...
      formData={formData}
      onConfirm={async (extraPayment, paymentMode, notes, checkInDate) => {
