...
function AdvanceBookingForm({isAdmin}) {
    ...
    **Grand Total (including GST): ₹${totalPrice.toFixed(2)}**`);
    ...
    useEffect(() => {
        ...
        const loadAvailableRooms = async () => {
          ...
          try {
            const rooms = await fetchAvailableRooms(formData.stay_info.durationOfStay, formData.stay_info.checkInDateTime,formData.stay_info.probableCheckOutDateTime);
            console.log(rooms);
    ...
    }, [formData.stay_info.checkInDateTime, formData.stay_info.probableCheckOutDateTime, formData.stay_info.durationOfStay]);  // Dependency array for re-running the effect
    ...
    const handleSubmit = async (e) => {
        ...
        const updatedFormData = {
            ...
            bookingStatus: "Confirmed", // Add bookingStatus
        };
